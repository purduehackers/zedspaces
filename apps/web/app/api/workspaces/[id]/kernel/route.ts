...
const kernelSchema = z.object({
  kernel: z.discriminatedUnion("kind", [
    z.object({ kind: z.literal("list") }).strict(),
    z.object({ kind: z.literal("python"), executable: path.nullable() }).strict(),
    z.object({ kind: z.literal("kernelspec"), name: z.string().min(1).max(255).regex(/^[a-zA-Z0-9._-]+$/) }).strict(),
  ]),
  cwd: path,
}).strict();
...
export const POST = handler<Request, WorkspaceParams>((req, ctx) => connectSandboxProcess(req, ctx, async () => {
  const { kernel, cwd } = await parseBody(req, kernelSchema, { maxBytes: 32_768 });
  return JSON.stringify({
    command: bundledPython,
    arguments: ["/usr/local/lib/zedspaces/kernel.py", JSON.stringify(kernel)],
    envs: {}, cwd: kernel.kind === "list" ? null : cwd, connection: null,
  });
}));
