...
export async function SiteShell({ children, reading = false }: { children: ReactNode; reading?: boolean }) {
  const viewer = await getViewer();
  const width = reading ? "max-w-3xl" : "max-w-6xl";
  return <div className="flex min-h-full flex-1 flex-col font-sans">
    ...
    <header className="border-b border-line">
      <div className={`mx-auto flex w-full ${width} flex-wrap items-center justify-between gap-x-8 gap-y-3 px-5 py-4 sm:px-8`}>
        ...
        </div> : <Link href="/login" className="brand-label px-3 py-2 text-text hover:bg-raised hover:text-yellow">Sign in</Link>}
      </div>
    </header>
    <main id="main" className={`mx-auto flex w-full ${width} flex-1 flex-col px-5 py-8 sm:px-8`}>{children}</main>
    <footer className="border-t border-line bg-panel text-text">
      <div className={`mx-auto flex w-full ${width} flex-wrap justify-between gap-3 px-5 py-6 font-mono text-sm sm:px-8`}>
