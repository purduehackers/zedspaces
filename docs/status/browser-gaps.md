...
| Accessibility / input | AccessKit-to-DOM adapter, labelled/full-document text mode, Tab escape and caret-anchored IME are deployed. File-tree/picker/tab/prompt semantics are being added. Real screen-reader/physical mobile checks and full international keyboard-layout mapping remain incomplete. | `zed/crates/gpui_web/src/accessibility.rs`, `zed/crates/gpui_web/src/ime_mirror.rs`, `zed/crates/gpui_web/src/events.rs` |
