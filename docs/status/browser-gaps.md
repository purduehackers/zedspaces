...
| Debugger | Private sandbox DAP transport is deployed. Live Node and Python launch, breakpoints, stack/variables, stepping and stopping pass. New-tab task replay/control labels are deployed. Native process picking and browser-local executables are not supported. | `zed/crates/zed_web/src/debugger.rs`, `sandbox/supervisor/src/debugger.rs` |
| Jupyter / REPL | Inline Python and the kernel image are deployed; live browser execution exposed a protocol-library clock panic. The correction passes native/WASM checks and awaits publication. Bundled Python or a project interpreter with ipykernel; no notebook editor or widget protocol. | `zed/crates/repl/src/kernels/web_kernel.rs`, `sandbox/image/kernel.py` |
| Calls and audio | Browser WebRTC audio/screen sharing and Zed-styled controls are implemented, not yet deployed. Temporary mixed-browser media checks pass. Optional Cloudflare TURN is unconfigured. No camera video, native channels/accounts, device picker or audio test window. | `apps/web/app/(editor)/w/[id]/browser-call.ts`, `apps/web/lib/call-room.ts`, `zed/crates/title_bar/src/collab_web.rs` |
...
| Clipboard | Browser text/image reads, native Copy/Cut/Paste events and async action replay are deployed. Text and PNG paste were verified live; image copy lacks a verified UI consumer. Secondary Vim/Helix register reads and macro suspension are implemented but await release and browser checks. | `zed/crates/gpui_web/src/clipboard.rs`, `zed/crates/zed_web/src/clipboard.rs` |
