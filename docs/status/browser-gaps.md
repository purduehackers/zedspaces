...
| Debugger | Private sandbox DAP transport is deployed. Live Node launch, breakpoints, stack/variables, stepping and disconnect pass; Python live breakpoint checks are in progress. New-tab task replay/control labels await the next release. Native process picking and browser-local executables are not supported. | `zed/crates/zed_web/src/debugger.rs`, `sandbox/supervisor/src/debugger.rs` |
| Jupyter / REPL | Inline Python is implemented and native/WASM/app checks plus Linux kernel protocol checks pass. Deployment and browser UI validation are pending. Bundled Python or a project interpreter with ipykernel; no notebook editor or widget protocol. | `zed/crates/repl/src/kernels/web_kernel.rs`, `sandbox/image/kernel.py` |
