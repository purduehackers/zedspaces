...
| Jupyter / REPL | Inline Python execution, Unicode, persistent values, rich output, errors and input pass live. The clock panic is fixed; an Interrupt control-reply correction is validated in the disposable VM and awaits publication. Bundled Python or a project interpreter with ipykernel; no notebook editor or widget protocol. | `zed/crates/repl/src/kernels/web_kernel.rs`, `sandbox/image/kernel.py` |
