...
mod tests {
    ...
    #[test]
    fn error_code_shapes() {
        assert_eq!(
            parse_error_code(r#"{"error":{"code":"slots_exhausted","message":"x"}}"#).as_deref(),
            Some("slots_exhausted")
        );
        assert_eq!(
            parse_error_code(r#"{"error":"no_free_slot"}"#).as_deref(),
            Some("no_free_slot")
        );
        ...
        assert!(matches!(
            classify(410, String::new()),
            ControlPlaneError::Retired
        ));
    }

    #[test]
    fn tarball_urls_are_restricted_to_public_https_hosts() {
        for ok in [
            "https://abc123.public.blob.vercel-storage.com/ws/restore.tgz?token=x",
            "https://example.com/x.tgz",
            "https://[2606:4700::1]/x.tgz",
            "https://8.8.8.8/x.tgz",
        ] {
            check_tarball_url(ok, false).unwrap_or_else(|error| panic!("{ok}: {error}"));
        }
        for bad in [
            "http://example.com/x.tgz",
            "https://127.0.0.1:8451/control/x",
            "https://169.254.169.254/latest/meta-data",
            "https://10.0.0.5/x.tgz",
            "https://192.168.1.1/x.tgz",
            "https://172.16.0.1/x.tgz",
            "https://100.64.0.1/x.tgz",
            "https://[::1]/x.tgz",
            "https://[fd00::1]/x.tgz",
            "https://[fe80::1]/x.tgz",
            "https://[::ffff:127.0.0.1]/x.tgz",
            "https://localhost/x.tgz",
            "https://blob/x.tgz",
            "https://host.docker.internal/x.tgz",
            "https://printer.local/x.tgz",
            "https://user:pw@example.com/x.tgz",
            "ftp://example.com/x.tgz",
            "not a url",
        ] {
            assert!(
                check_tarball_url(bad, false).is_err(),
                "{bad} must be refused"
            );
        }
        // The local docker test streams from the mock over plain http on a bare host name.
        check_tarball_url("http://blob/x.tgz", true).unwrap();
        check_tarball_url("http://127.0.0.1:9977/__blob/restore.tgz", true).unwrap();
        assert!(check_tarball_url("ftp://blob/x.tgz", true).is_err());
    }

    #[test]
    fn error_bodies_are_capped() {
        let huge = format!(
            r#"{{"error":{{"code":"nope","message":"{}"}}}}"#,
            "m".repeat(10_000)
        );
        match classify(409, huge) {
            ControlPlaneError::Status { code, body, .. } => {
                assert_eq!(
                    code.as_deref(),
                    Some("nope"),
                    "the code is parsed before the cap"
                );
    ...
    fn backoff_grows_and_stays_bounded() {
        // Enough draws to hit both ends of the jitter range (the upper bound used to be
        // exceeded by one millisecond about once in 126 draws).
        for _ in 0..2_000 {
            for attempt in 1..=5 {
                let base = BACKOFF_BASE.as_millis() as u64 * (1u64 << (attempt - 1));
                let pause = backoff(attempt).as_millis() as u64;
                // 250 ms · 2ⁿ⁻¹ ± 25 %.
                assert!(pause >= base - base / 4, "attempt {attempt}: {pause}ms");
                assert!(pause <= base + base / 4, "attempt {attempt}: {pause}ms");
            }
        }
        // The exponent is capped, so a long retry chain never overflows.
        assert!(backoff(40).as_millis() <= 250 * 64 + 250 * 16);
    }
    ...
    fn retry_after_is_parsed_and_clamped() {
        ...
        headers.insert(
            ...
            "Wed, 02 Sep 2026 12:00:00 GMT".parse().unwrap(),
        );
        assert_eq!(retry_after(&headers), None);
    }

    #[test]
    fn directive_accepts_d29_shape() {
        let directive: ActivityDirective = serde_json::from_str(
            r#"{"idleStopAt":null,"sessionCapAt":1756886400000,"stop":false,"forwards":[],"serverTime":1756800000000}"#,
        )
        .unwrap();
        assert_eq!(directive.server_time, Some(1756800000000));
        assert_eq!(directive.forwards, Some(vec![]));
        let minimal: ActivityDirective =
            serde_json::from_str(r#"{"idleStopAt":1,"sessionCapAt":2}"#).unwrap();
        assert!(!minimal.stop);
        assert!(minimal.forwards.is_none());
    }
}
