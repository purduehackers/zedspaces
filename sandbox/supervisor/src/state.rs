...
mod tests {
    ...
    fn private(port: u16, slot: Option<u16>) -> Forward {
        Forward {
            port,
    ...
    #[test]
    fn forwards_slot_binding() {
        let forwards = ForwardsState::new();
        let mut changes = forwards.changed();
        forwards.insert(private(3000, None));
        assert!(changes.has_changed().unwrap());
        changes.mark_unchanged();
        forwards.insert(private(3000, None));
        assert!(!changes.has_changed().unwrap());
        assert_eq!(forwards.bind_slot(8444, 3000), Ok(()));
        assert_eq!(forwards.slot_port(8444), Some(3000));
        assert_eq!(forwards.current()[0].slot, Some(8444));
        assert_eq!(
            forwards.bind_slot(8444, 4000),
            Err(SlotError::NotPrivateForward(4000))
        );
        forwards.insert(private(4000, None));
        assert_eq!(
            forwards.bind_slot(8444, 4000),
        ...
        assert_eq!(
            forwards.bind_slot(9999, 4000),
            Err(SlotError::UnknownSlot(9999))
        );
        // A token for a port that another slot already serves must not move the binding.
        assert_eq!(
            forwards.bind_slot(8445, 3000),
            Err(SlotError::PortBoundElsewhere {
                port: 3000,
                slot: 8444
            })
        );
        assert_eq!(forwards.slot_port(8444), Some(3000));
        assert_eq!(forwards.slot_port(8445), None);
        forwards.remove(3000);
        assert_eq!(forwards.slot_port(8444), None);
        forwards.replace(vec![private(4000, Some(8445))]);
        assert_eq!(forwards.slots()[&8445], Some(4000));
        assert_eq!(forwards.slots()[&8444], None);
    }

    #[test]
    fn a_directive_older_than_a_local_insert_is_ignored() {
        let forwards = ForwardsState::new();
        forwards.replace(vec![private(3000, Some(8444))]);
        let snapshot = forwards.generation();
        // The control plane's list from before the insert must not wipe the new forward.
        forwards.insert(private(4000, Some(8445)));
        assert!(!forwards.replace_if_unchanged(vec![private(3000, Some(8444))], Some(snapshot)));
        assert_eq!(forwards.current().len(), 2);
        assert_eq!(forwards.slot_port(8445), Some(4000));
        // With a current generation the directive applies.
        let snapshot = forwards.generation();
        assert!(forwards.replace_if_unchanged(vec![private(3000, Some(8444))], Some(snapshot)));
        assert_eq!(forwards.current().len(), 1);
        assert_eq!(forwards.slot_port(8445), None);
        // An equal list is a no-op that still reports success and does not bump the generation.
        let snapshot = forwards.generation();
        assert!(forwards.replace_if_unchanged(vec![private(3000, Some(8444))], Some(snapshot)));
        assert_eq!(forwards.generation(), snapshot);
    }
    ...
    fn health_bodies() {
        ...
        state.update(|inner| {
            ...
            inner.server.started_at_ms = Some(7);
        });
        // Alive but not yet accepting: neither body may claim the server is up.
        let json = serde_json::to_value(state.minimal()).unwrap();
        assert_eq!(json["serverUp"], false);
        let report = state.snapshot(false, &[], &[]);
        assert!(!report.server.status.running);
        assert!(!report.server.status.listening);
        let mut up = state.server_up_watch();
        state.mark_server_listening();
        assert_eq!(*up.borrow_and_update(), Some(7));
        let report = state.snapshot(false, &[private(3000, Some(8444))], &[3000]);
        assert!(report.forwards.is_none());
        assert!(report.server.status.running);
        assert!(report.server.status.listening);
        assert_eq!(report.proxy.slots[&8444], Some(3000));
        assert_eq!(report.proxy.slots[&8445], None);
        let json = serde_json::to_value(state.minimal()).unwrap();
        assert_eq!(json["ok"], true);
        assert_eq!(json["serverUp"], true);
        assert_eq!(json["phase"], "ready");
        assert!(json.get("uptimeSec").is_some());
        let json = serde_json::to_value(&report).unwrap();
        assert_eq!(json["server"]["running"], true);
        assert_eq!(json["server"]["listening"], true);
    }

    #[test]
    fn server_exit_clears_the_stale_health() {
        let state = AgentState::new("b".to_string(), None);
        state.update(|inner| {
            inner.server.running = true;
            inner.server.pid = Some(42);
            inner.server.started_at_ms = Some(1);
            inner.server_health = Some(ServerHealth {
                build: "b".into(),
                version: "v".into(),
                uptime_secs: 10,
                session_active: true,
                last_input_at: Some(5),
                session: None,
                dirty_buffers: None,
            });
        });
        state.mark_server_listening();
        let mut up = state.server_up_watch();
        assert_eq!(*up.borrow_and_update(), Some(1));
        state.mark_server_exited(Some(1));
        assert_eq!(*up.borrow_and_update(), None);
        state.read(|inner| {
            assert!(!inner.server.running);
            assert!(!inner.server.listening);
            assert_eq!(inner.server.pid, None);
            assert_eq!(inner.server.last_exit, Some(1));
            assert!(
                inner.server_health.is_none(),
                "a dead server has no session"
            );
        });
        // Listening cannot be claimed for a child that is not running.
        state.mark_server_listening();
        assert!(state.read(|inner| !inner.server.listening));
    }

    #[test]
    fn busy_counts_overlapping_commands() {
        let state = AgentState::new("b".to_string(), None);
        assert!(!state.busy());
        ...
        assert!(
            state.busy(),
            "postAttach finishing must not clear postCreate's busy"
        );
        drop(second);
        assert!(!state.busy());
    }
