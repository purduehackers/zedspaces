...
mod tests {
    ...
    #[test]
    fn foreign_port_owner_is_told_from_the_server() {
        let bin = Path::new("/usr/local/bin/zed-remote-server");
        assert!(names_server_binary("zed-remote-server", bin));
        // `/proc/<pid>/comm` is truncated to 15 bytes.
        assert!(names_server_binary("zed-remote-serv", bin));
        assert!(names_server_binary(
            "/opt/zed/target/debug/zed-remote-server",
            bin
        ));
        assert!(!names_server_binary("node", bin));
        assert!(!names_server_binary("cargo", bin));
        assert!(!names_server_binary("", bin));
        let debug_bin = Path::new("/repo/zed/target/debug/remote_server");
        assert!(names_server_binary("remote_server", debug_bin));
        assert!(!names_server_binary("remote_server_test", Path::new("")));
    }

    #[test]
    fn command_line_matches_serve_args() {
        let spec = ServerSpec {
            bin: PathBuf::from("/usr/local/bin/zed-remote-server"),
            listen: "0.0.0.0:8443".parse().unwrap(),
            jwt_key_files: vec![PathBuf::from("/vercel/.zs/jwt/key-0.pem")],
            workspace_id: "ws_1".to_string(),
            audience: "sb-1".to_string(),
            issuer: "zs".to_string(),
            workspace_root: PathBuf::from("/workspaces/api"),
            client_build: "c3cf80c-42".to_string(),
            allowed_origins: vec!["https://zs.example.com".to_string()],
            control_secret_file: PathBuf::from("/vercel/.zs/run/control.secret"),
            control_listen: "127.0.0.1:8451".parse().unwrap(),
            supervisor_url: "http://127.0.0.1:8450".to_string(),
            port_file: PathBuf::from("/vercel/.zs/run/server.port"),
            env: BTreeMap::from([("SHELL".to_string(), "/bin/bash".to_string())]),
            cwd: PathBuf::from("/workspaces/api"),
        };
        let argv = spec.argv().join(" ");
        assert_eq!(
            argv,
            "serve --listen 0.0.0.0:8443 --jwt-public-key /vercel/.zs/jwt/key-0.pem --workspace-id ws_1 \
             --audience sb-1 --issuer zs --workspace-root /workspaces/api --client-build c3cf80c-42 \
             --allowed-origin https://zs.example.com --control-secret-file /vercel/.zs/run/control.secret \
             --control-listen 127.0.0.1:8451 --supervisor-url http://127.0.0.1:8450 --port-file /vercel/.zs/run/server.port"
        );
        assert!(!spec.env.contains_key("ZS_CONTROL_SECRET"));
        let _ = spec.command();
    }

    #[test]
    fn child_env_strips_credentials_and_fills_d28_variables() {
        let env = BTreeMap::from([
            ("ZS_CONTROL_URL", "https://zs.example.com/api".to_string()),
            ("ZS_SANDBOX_TOKEN", "zsb_test".to_string()),
            ("ZS_SANDBOX_NAME", "sb-1".to_string()),
            ("ZS_WORKSPACE_ID", "ws_1".to_string()),
            ("HOME", "/vercel".to_string()),
        ]);
        let config = crate::config::Config::from_lookup(|key| env.get(key).cloned()).unwrap();
        let base: Vec<(String, String)> = [
            ("ZS_SANDBOX_TOKEN", "zsb_leak"),
            ("ZS_CONTROL_URL", "https://zs.example.com/api"),
            ("ZS_CONTROL_PLANE_URL", "https://zs.example.com"),
            ("ZS_BYPASS_SECRET", "bypass"),
            ("ZS_CONTROL_SECRET", "never"),
            ("NPM_TOKEN", "npm_secret_inherited_untouched"),
            ("HOME", "/vercel"),
        ]
        .into_iter()
        ...
        let manifest_env = BTreeMap::from([
            ...
            ("ZS_REGION".to_string(), "iad1".to_string()),
        ...
        let child = config.child_env_from(base, &manifest_env, &remote_env);
        for stripped in [
            "ZS_SANDBOX_TOKEN",
            "ZS_CONTROL_URL",
            ...
            assert!(
                !child.contains_key(stripped),
                "{stripped} must not reach a child"
            );
        }
        assert_eq!(child["ZS_SUPERVISOR_URL"], "http://127.0.0.1:8450");
        assert_eq!(
            child["ZS_CONTROL_SECRET_FILE"],
            "/vercel/.zs/run/control.secret"
        );
        assert_eq!(child["NODE_ENV"], "development");
        assert_eq!(child["ZS_REGION"], "iad1");
        assert_eq!(child["APP_MODE"], "dev");
        assert_eq!(child["NPM_TOKEN"], "npm_secret_inherited_untouched");
        // D28: filled when missing, never overridden by a repository's remoteEnv. `USER` is the
        // runtime user (`ubuntu` in the image; whoever runs the tests here).
        assert_eq!(child["SHELL"], "/bin/bash");
        assert_eq!(child["USER"], crate::config::runtime_user_name());
        assert_eq!(child["HOME"], "/vercel");
        assert_eq!(child["LANG"], "C.UTF-8");
        assert!(child["PATH"].contains("/usr/bin"));
    }

    #[test]
    fn ports_body_maps_null_url() {
        let forwards = [
            Forward {
                port: 3000,
            ...
            Forward {
                ...
                label: None,
                url: Some("https://x.vercel.run".to_string()),
                slot: None,
            },
        ];
        ...
        assert_eq!(json["ports"][0]["process_name"], "node");
        assert!(
            json["ports"][0].get("loopback_only").is_none(),
            "b4's PortsBody has no such field yet"
        );
        assert_eq!(json["forwards"][0]["url"], "");
        assert_eq!(json["forwards"][0]["visibility"], "private");
        assert_eq!(json["forwards"][1]["url"], "https://x.vercel.run");
    }

    #[test]
    fn crash_tail_is_cut_on_a_char_boundary() {
        // 64 KiB of ASCII, then `…` straddling the cut: the old `String::truncate` panicked.
        let mut tail = "x".repeat(CRASH_TAIL_BYTES - 1);
        tail.push('…');
        tail.push_str("more");
        let mut cut = crate::logs::scrub(&tail).into_owned();
        crate::logs::truncate_at_boundary(&mut cut, CRASH_TAIL_BYTES);
        assert_eq!(cut.len(), CRASH_TAIL_BYTES - 1);
        assert!(cut.chars().all(|c| c == 'x'));
    }

    #[test]
    fn extensions_body_shape() {
        let install = vec!["toml".to_string()];
        assert_eq!(
            serde_json::to_string(&ExtensionsBody { install: &install }).unwrap(),
            r#"{"install":["toml"]}"#
        );
    }

    #[test]
    fn control_targets_follow_d21() {
        let control = ServerControl::new(
            "127.0.0.1:8443".parse().unwrap(),
            "127.0.0.1:8451".parse().unwrap(),
            SecretString::from("secret"),
        )
        .unwrap();
        assert_eq!(control.health_base(), "http://127.0.0.1:8443");
        assert_eq!(control.control_base(), "http://127.0.0.1:8451");
        assert_eq!(control.bearer(), "secret");
    }

    #[test]
    fn control_secret_file_is_private() {
        use std::os::unix::fs::PermissionsExt as _;
        let dir = tempfile::tempdir().unwrap();
        let secret = Supervisor::generate_control_secret();
        assert_eq!(secret.expose_secret().len(), 64);
        let path = Supervisor::write_control_secret(&secret, dir.path()).unwrap();
