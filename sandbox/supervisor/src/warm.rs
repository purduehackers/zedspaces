...
mod tests {
    ...
    #[test]
    fn probe_file_selection() {
        let dir = tempfile::tempdir().unwrap();
        let root = dir.path();
        for (path, contents) in [
            ("src/main.rs", "fn main() {}"),
            ("node_modules/x.ts", "export {}"),
            ("web/app.ts", "export {}"),
            ("README.md", "# hi"),
        ...
        let probes = pick_probe_files(root, &[], &[]);
        let names: Vec<String> = probes
            .iter()
            .map(|path| path.to_string_lossy().replace('\\', "/"))
            .collect();
        assert_eq!(names, vec!["src/main.rs", "web/app.ts", "README.md"]);
        assert!(probes.len() <= WARM_MAX_FILES);
    }

    #[test]
    fn probe_hints_first() {
        // b10 §6.5: hints open first (existing, relative, no `..`); a Feature adds an implied
        // probe when the table walk found none, even below the default depth.
        let dir = tempfile::tempdir().unwrap();
        let root = dir.path();
        for (path, contents) in [
            ("src/main.rs", "fn main() {}"),
            ("README.md", "# hi"),
            ("a/b/c/d/e/f/g/h/deep.ts", "export {}"),
        ] {
            let full = root.join(path);
            std::fs::create_dir_all(full.parent().unwrap()).unwrap();
            std::fs::write(full, contents).unwrap();
        ...
        let probes = pick_probe_files(root, &hints, &[]);
        let names: Vec<String> = probes
            .iter()
            .map(|path| path.to_string_lossy().replace('\\', "/"))
            .collect();
        ...
        let probes = pick_probe_files(root, &[], &["node"]);
        let names: Vec<String> = probes
            .iter()
            .map(|path| path.to_string_lossy().replace('\\', "/"))
            .collect();
        assert_eq!(
            names,
            vec!["src/main.rs", "a/b/c/d/e/f/g/h/deep.ts", "README.md"]
        );
    }

    #[test]
    fn probe_index_matches_names_and_extensions() {
        assert_eq!(probe_index("main.rs"), Some(0));
        assert_eq!(probe_index("Dockerfile"), Some(20));
        assert_eq!(probe_index("Makefile"), None);
        assert_eq!(probe_index("noextension"), None);
    }
    ...
    fn hello_frame_shape() {
        let hello = Hello {
            ...
            session_id: "warm-abc".into(),
            identifier: "zs-agent-warm/n1".into(),
            instance: "n1".into(),
            reconnect: false,
            takeover: false,
            client: "desktop",
        ...
        assert_eq!(
            ...
            r#"{"type":"hello","protocol":1,"build":"b1","workspace_id":"ws_1","session_id":"warm-abc","identifier":"zs-agent-warm/n1","instance":"n1","reconnect":false,"takeover":false,"client":"desktop","epoch":null}"#
        );
    }
    ...
    fn envelope_framing_roundtrip() {
        let envelope = messages::Envelope {
            id: 7,
            payload: Some(messages::envelope::Payload::AddWorktree(
                messages::AddWorktree {
                    path: "/workspaces/api".into(),
                    project_id: 0,
                    visible: true,
                },
            )),
        ...
        match decoded.payload {
            Some(messages::envelope::Payload::AddWorktree(worktree)) => {
                assert_eq!(worktree.path, "/workspaces/api");
                assert!(worktree.visible);
            }
            other => panic!("unexpected payload {other:?}"),
        }

        // An envelope whose oneof tag this subset does not name still decodes, with no payload.
        let mut unknown = Vec::new();
        prost::encoding::encode_key(1, prost::encoding::WireType::Varint, &mut unknown);
        prost::encoding::encode_varint(9, &mut unknown);
        prost::encoding::encode_key(
            999,
            prost::encoding::WireType::LengthDelimited,
            &mut unknown,
        );
        prost::encoding::encode_varint(0, &mut unknown);
        ...
        assert!(decode_envelope_frame(&[0, 0]).is_err());
        assert!(decode_envelope_frame(&[9, 0, 0, 0, 1]).is_err());
    }

    #[test]
    fn warm_token_claims() {
        let key = generate_warm_key().unwrap();
        assert!(key.public_pem.starts_with("-----BEGIN PUBLIC KEY-----"));
        let minted = mint_token(&key, "zs", "sb-1", "ws_1", 1_756_800_000).unwrap();
        assert!(
            minted
                .token
                .bytes()
                .all(|byte| byte.is_ascii_alphanumeric() || b"-._~+/=".contains(&byte)),
            "the token must be an RFC 2616 token: it rides in Sec-WebSocket-Protocol"
        );

        let mut validation = jsonwebtoken::Validation::new(jsonwebtoken::Algorithm::ES256);
        validation.set_issuer(&["zs"]);
        validation.set_audience(&["sb-1"]);
        validation.validate_exp = false;
        let decoded = jsonwebtoken::decode::<WarmClaims>(
            &minted.token,
            &jsonwebtoken::DecodingKey::from_ec_pem(key.public_pem.as_bytes()).unwrap(),
            &validation,
        )
        .unwrap();
        assert_eq!(decoded.claims.iss, "zs");
        assert_eq!(decoded.claims.aud, "sb-1");
        assert_eq!(decoded.claims.sub, "zs-agent-warm");
        assert_eq!(decoded.claims.ws, "ws_1");
        assert_eq!(decoded.claims.sid, minted.session_id);
        assert_eq!(decoded.claims.sid, format!("warm-{}", decoded.claims.jti));
    ...
    fn ws_url_forms() {
        assert_eq!(
            rpc_ws_url("http://127.0.0.1:8443").unwrap(),
            (
                "ws://127.0.0.1:8443/rpc".to_string(),
                "127.0.0.1".to_string(),
                8443
            )
        );
        assert!(rpc_ws_url("https://example.com").is_err());
    }
}
