...
mod tests {
    ...
    fn proc_net_parsing() {
        ...
        let text = "  sl  local_address rem_address   st tx_queue rx_queue tr tm->when retrnsmt   uid  timeout inode\n\
                    ...
                    \x20  1: 0100007F:0BB9 00000000:0000 01 00000000:00000000 00:00000000 00000000  1000        0 12346 1 0000000000000000 100 0 0 10 0\n";
    ...
    fn line(index: usize, address: &str, state: &str, uid: u32, inode: u64) -> String {
        format!(
            "  {index}: {address} 00000000:0000 {state} 00000000:00000000 00:00000000 00000000  \
             {uid}        0 {inode} 1 0000000000000000 100 0 0 10 0\n"
        )
    }

    fn table(rows: &[String]) -> String {
        let mut text = "  sl  local_address rem_address   st tx_queue rx_queue tr tm->when \
    ...
    #[test]
    fn parses_ipv6_listen_line() {
        let wildcard = parse_proc_net_tcp(
            &table(&[line(
                0,
        ...
        assert_eq!(wildcard[0].port, 8080);
        assert!(!wildcard[0].loopback_only);
        assert!(wildcard[0].v6 && !wildcard[0].v4);
        let loopback = parse_proc_net_tcp(
            &table(&[line(
                0,
        ...
        assert!(loopback[0].loopback_only);
    }

    #[test]
    fn unions_v4_and_v6_by_port() {
        let mut sockets =
            parse_proc_net_tcp(&table(&[line(0, "0100007F:0BB8", "0A", 1000, 1)]), false);
        sockets.extend(parse_proc_net_tcp(
            &table(&[line(
                0,
        ...
        let union = union_by_port(sockets);
        assert_eq!(union.len(), 1);
        let socket = &union[&3000];
        assert!(socket.v4 && socket.v6);
        // One wildcard listener means the port is reachable from outside loopback.
        assert!(!socket.loopback_only);
    }

    #[test]
    fn v6_only_is_flagged() {
        let union = union_by_port(parse_proc_net_tcp(
            &table(&[line(
                0,
                "00000000000000000000000000000000:1388",
                "0A",
                1000,
                3,
            )]),
        ...
        let socket = &union[&5000];
        assert!(!socket.v4 && socket.v6);
    }

    #[test]
    fn parse_local_address_rejects_bad_input() {
        assert_eq!(parse_local_address("0100007:0BB8", false), None);
        assert_eq!(parse_local_address("zzzzzzzz:0BB8", false), None);
        assert_eq!(parse_local_address("0100007F", false), None);
        assert_eq!(parse_local_address("0100007F:ZZZZ", false), None);
        assert_eq!(parse_local_address("0100007F:0BB8", true), None);
    }

    #[test]
    fn socket_inode_links() {
    ...
    #[test]
    fn excludes_infra_ports_and_self() {
        let mut rows = Vec::new();
        // 8443..=8451 (the infra set), a user port on 3000 and one on 5173 owned by "us".
        for (index, port) in (8443u16..=8451).enumerate() {
            rows.push(line(
                index,
        ...
        rows.push(line(20, "00000000:0BB8", "0A", 1000, 900));
        rows.push(line(21, "0100007F:1435", "0A", 1000, 901));
        let sockets = parse_proc_net_tcp(&table(&rows), false);
        let by_port = union_by_port(sockets);
        assert_eq!(by_port.len(), 11);
        let own = BTreeSet::from([901u64]);
        let visible = user_ports(by_port, &own);
        assert_eq!(visible.keys().copied().collect::<Vec<u16>>(), vec![3000]);
        assert!(visible.keys().all(|port| !is_infra_port(*port)));
    }

    #[test]
    fn lsof_listen_table_parsing() {
        let text = "p5140\ncrapportd\nf11\ntIPv4\nn*:63335\nf12\ntIPv6\nn*:63335\n\
                    p5498\ncfigma_agent\nf8\ntIPv4\nn127.0.0.1:44950\n\
                    p77\ncnode\nf3\ntIPv6\nn[::1]:5173\nf4\ntIPv4\nnbogus\n";
        let listeners = lsof::parse_listen_table(text);
        assert_eq!(listeners.len(), 4);
        assert_eq!(listeners[0].pid, 5140);
        assert_eq!(listeners[0].command, "rapportd");
        assert!(!listeners[0].v6 && listeners[0].ip.is_none());
        assert_eq!(listeners[0].port, 63335);
        assert!(listeners[1].v6);
        assert_eq!(listeners[2].ip, Some(IpAddr::V4(Ipv4Addr::LOCALHOST)));
        assert_eq!(listeners[2].port, 44950);
        assert_eq!(listeners[3].ip, Some(IpAddr::V6(Ipv6Addr::LOCALHOST)));
        assert!(listeners[3].v6);
        assert_eq!(lsof::pid_of_key(listeners[3].key()), 77);
        let socket = listeners[2].socket();
        assert!(socket.loopback_only && socket.v4 && !socket.v6);
        let wildcard = listeners[0].socket();
        assert!(!wildcard.loopback_only);
        let union = union_by_port(listeners.iter().map(lsof::LsofListener::socket).collect());
        assert_eq!(union.len(), 3);
        assert!(union[&63335].v4 && union[&63335].v6);
        assert_eq!(lsof::parse_name("*:8080"), Some((None, 8080)));
        assert_eq!(lsof::parse_name("nonsense"), None);
        assert_eq!(lsof::parse_name("127.0.0.1:x"), None);
    }

    #[test]
    fn diff_and_infra() {
        let previous = BTreeSet::from([3000, 4000]);
        let current = BTreeSet::from([4000, 5000]);
        assert_eq!(
            diff_ports(&previous, &current),
            PortDiff {
                added: vec![5000],
                removed: vec![3000]
            }
        );
        assert!(is_infra_port(8443));
        assert!(is_infra_port(8451));
        assert!(!is_infra_port(8452));
        assert!(!is_infra_port(3000));
    }
}
